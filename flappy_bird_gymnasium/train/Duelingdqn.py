...
batch_size = 64
buffer_size = 50000
update_target_every = 1000  # update frequency
epsilon_start = 1.0
epsilon_end = 0.01
epsilon_decay = 0.995
...
class DuelingQNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim):
        ...
        self.advantage = nn.Linear(hidden_dim, action_dim)  # A(s,a)

    def forward(self, x):
        x = self.feature(x)
        value = self.value(x)
        advantage = self.advantage(x)
        q = value + advantage - advantage.mean(dim=1, keepdim=True)  # combine
        return q


def run_dqn(seed):
    q_net = DuelingQNetwork(state_dim, action_dim, hidden_dim)
    target_net = DuelingQNetwork(state_dim, action_dim, hidden_dim)
    ...
    while total_steps < max_steps:
        ...
        while not done:
            if np.random.rand() < epsilon:
                ...
                with torch.no_grad():
                    q_values = q_net(torch.FloatTensor(state).unsqueeze(0))  # [1, state_dim]
        ...
        if episode_num % 50 == 0:
            ...
            torch.save(q_net.state_dict(), f'{MODEL_PATH}/DQN_dueling_qnet.pth')
...
if __name__ == "__main__":
    ...
    df.to_csv('./results/dqn_dueling_episode_results.csv', index=False)

    print("\nResults saved to ./results/dqn_dueling_episode_results.csv")
    print("\nAvg Reward Summary:")
