...
def run_dueling_double_dqn(seed):
    ...
    while total_steps < max_steps:
        ...
        if episode_num % 50 == 0:
            ...
            torch.save(q_net.state_dict(), f'{MODEL_PATH}/DQN_dueling_double_qnet.pth')
...
if __name__ == "__main__":
    ...
    for run in range(NUM_RUNS):
        episode_data = run_dueling_double_dqn(seed=run)
        all_episode_data.extend(episode_data)

    df = pd.DataFrame(all_episode_data)

    os.makedirs('./results', exist_ok=True)
    df.to_csv('./results/dqn_dueling_double_episode_results.csv', index=False)

    print("\nResults saved to ./results/dqn_dueling_double_episode_results.csv")
    print("\nAvg Reward Summary:")
