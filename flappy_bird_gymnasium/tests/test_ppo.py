...
def run_ppo(epoch=500, audio_on=False, render_mode="human", model_name="tiiuae/falcon-rw-1b", llm=False):
    ...
    policy_path = os.path.join(MODEL_PATH, "PPO_policy_pcg.pth")
    ...
    for t in range(epoch):
        ...
        scores.append(score)
        print(f"Episode {t + 1}: Score = {score}, Total Reward = {total_reward:.2f}")
    
    run_avg = np.mean(scores)
    print(f"Average Score = {run_avg:.2f}")

    env.close()
    return run_avg

if __name__ == "__main__":
    results = [run_ppo() for _ in range(5)]
