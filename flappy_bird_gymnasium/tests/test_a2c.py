...
from flappy_bird_gymnasium.train.A2C import Actor, Critic
...
def play(epoch=500, audio_on=False, render_mode="human", model_name="tiiuae/falcon-rw-1b", llm=False):

    env = gymnasium.make("FlappyBird-pcg-v0", audio_on=audio_on, use_lidar=False, render_mode=render_mode)

    actor = Actor(state_dim=env.observation_space.shape[0], action_dim=env.action_space.n, hidden_dim=128)
    critic = Critic(state_dim=env.observation_space.shape[0], hidden_dim=128)

    actor.load_state_dict(torch.load(os.path.join(MODEL_PATH, "A2C_actor.pth"), map_location='cpu'))
    critic.load_state_dict(torch.load(os.path.join(MODEL_PATH, "A2C_critic.pth"), map_location='cpu'))
    ...
    for t in range(epoch):
        ...
        while not done:
            ...
            action, _, _ = actor.act(state_tensor)
